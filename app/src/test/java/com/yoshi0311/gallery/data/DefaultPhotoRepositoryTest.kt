...
/**
 * Unit tests for [DefaultPhotoRepository].
 */
@OptIn(ExperimentalCoroutinesApi::class) // TODO: Remove when stable
class DefaultPhotoRepositoryTest {

    @Test
    fun photos_newItemSaved_itemIsReturned() = runTest {
        val repository = DefaultPhotoRepository(FakePhotoDao())

        repository.add("Repository")

        assertEquals(repository.photos.first().size, 1)
    }
...
private class FakePhotoDao : PhotoDao {
    ...
    override suspend fun insertPhoto(item: Photo) {
        data.add(0, item)
    }
}
